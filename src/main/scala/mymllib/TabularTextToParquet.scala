...
object TabularTextToParquet {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val cleveland = spark.sparkContext.textFile(dataPath)
    val rowRDD = cleveland.map(_.split("""\s+""")).map(p => Row.fromSeq(p.toSeq))
    var clevelandDataFrame = spark.sqlContext.createDataFrame(rowRDD, orischema)
    ...
    println(clevelandDataFrame.select("label").map(a => (a, 1)))
//     .reduceByKey(_ + _).getClass)
    clevelandDataFrame.select("label").map(a => (a, 1))
